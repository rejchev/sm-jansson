...
{
    return !isOK() ? jtInvalid : innerTypeConversion(m_pJson->type);
}
...
IJson *Json::find(const JsonType &type, const std::function<bool(const IJson*)>& condition) const
{
    ...
    json_t* pBuffer = nullptr;

    if(selfType == jtObject)
    {
        const char* key;

        for(void* iter = json_object_iter(m_pJson);
            pBuffer == nullptr && (key = json_object_iter_key(iter));
            iter = json_object_iter_next(m_pJson, iter))
        {
    ...
    {
        for(size_t i = 0; pBuffer == nullptr && i < size(); i++)
        {
    ...
    return (pBuffer != nullptr) ? new Json(pBuffer) : nullptr;
}

JsonType Json::innerTypeConversion(const json_type &type) {
    switch (type) {
